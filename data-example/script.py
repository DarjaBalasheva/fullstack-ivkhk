            ...
            for group in os.listdir(YEAR_PATH.format(year=year_)):
                if os.path.isdir(GROUP_PATH.format(year=year_, group=group)):
                    if group != "inspectionProfiles":
                        sql = """INSERT IGNORE INTO group_list (
                            group_year, group_)
                            VALUES (%s, %s);"""
                        ...
                        if group_id == 0:  # Group already exist
                            sql = """SELECT group_uuid FROM group_list WHERE
                                    group_year=%s
                                    AND group_=%s"""
                    ...
                    for project in os.listdir(
                        GROUP_PATH.format(year=year_, group=group)
                    ):
                        if (
                            os.path.isdir(
                                PROJECT_PATH.format(
                            ...
                            with open(
                                f"{PROJECT_PATH.format(year=year_, group=group, project=project)}/prop.json",
                                "r",
                            ) as file:
                                info = json.load(file)
                                # bb = aa.replace('\n', ';').split(sep=';')
                            ...
                            sql = """INSERT IGNORE INTO project_info (project_name, year_,
                                    group_uuid, type_uuid, is_best)
                                         VALUES (%s, %s , %s, %s, %s);"""
                            mycursor.execute(
                            ...
                            for name in info["name"]:
                                ...
                                sql = """INSERT IGNORE INTO students (first_name, last_name, project_uuid)
                                             VALUES (%s, %s, %s);"""
                                mycursor.execute(
                                    sql,
                                    (
                            ...
                            for media_type in os.listdir(
                                f"{PROJECT_PATH.format(year=year_, group=group, project=project)}/"
                            ):
                                if (
                                    os.path.isdir(
                                        f"{PROJECT_PATH.format(year=year_, group=group, project=project)}/{media_type}"
                                    )
                                    and media_type[0] != "."
                                ):
                                    for file in os.listdir(
                                        ...
                                        if file[0] != ".":
                                            ...
                                            print(
                                                "!!!!!!!!",
                                                len(
                                                    files_dict_for_insert["media_file"]
                                                ),
                                            )
                            ...
                            app_values = [
                                _ for _ in files_dict_for_insert["media_file"]
                            ]
                            app_values.append(files_dict_for_insert["project_file"][0])
                            app_values.append(project_id)

                            sql = f"""INSERT IGNORE INTO files ({app_columns}, pdf_file, project_uuid)
