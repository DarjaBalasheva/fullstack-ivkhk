...
def create_db():
    ...
    mycursor.execute(
        ...
        group_uuid VARCHAR(36) DEFAULT (UUID()) NOT NULL PRIMARY KEY,
        group_ VARCHAR(100),
        group_year YEAR)"""
    )
    mydb.commit()
    mycursor.execute(
        ...
        type_uuid VARCHAR(36) DEFAULT (UUID()) NOT NULL PRIMARY KEY,
        type_ VARCHAR(100))"""
    )
    mydb.commit()
    mycursor.execute(
    ...
    mycursor.execute(
        ...
        project_uuid int,
        CONSTRAINT FK_project_uuid FOREIGN KEY (project_uuid)
        REFERENCES project_info(project_uuid));
    """
    )
    mydb.commit()
    mycursor.execute(
    ...
    CREATE TABLE IF NOT EXISTS files (
      ...
      app_24 VARCHAR(300),
      app_25 VARCHAR(300),
     CONSTRAINT FK_project_uuid_ FOREIGN KEY (project_uuid)
     REFERENCES project_info(project_uuid)
    );
    """
    )
